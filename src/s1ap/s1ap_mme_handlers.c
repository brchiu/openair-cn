...
s1ap_mme_handle_s1_setup_request (
    ...
    S1AP_S1AP_PDU_t *pdu)
{
  S1AP_S1SetupRequest_t                  *container;
  ...
  if (ie) {
    OAILOG_MESSAGE_ADD (context, "%*s ", (int)ie->value.choice.ENBname.size, ie->value.choice.ENBname.buf);
    enb_name = (char *) ie->value.choice.ENBname.buf;

    if (enb_name != NULL) {
      memcpy(enb_association->enb_name,  ie->value.choice.ENBname.buf,  ie->value.choice.ENBname.size);
      enb_association->enb_name[ie->value.choice.ENBname.size] = '\0';
    }
  }
    ...
    S1AP_S1AP_PDU_t *pdu)
{
  ...
  eRABSetupItemCtxtSURes_p = (S1AP_E_RABSetupItemCtxtSUResIEs_t *)
    ie->value.choice.E_RABSetupListCtxtSURes.list.array[0];
  MME_APP_INITIAL_CONTEXT_SETUP_RSP (message_p).mme_ue_s1ap_id = ue_ref_p->mme_ue_s1ap_id;
  MME_APP_INITIAL_CONTEXT_SETUP_RSP (message_p).eps_bearer_id = eRABSetupItemCtxtSURes_p->value.choice.E_RABSetupItemCtxtSURes.e_RAB_ID;
  ...
  MME_APP_INITIAL_CONTEXT_SETUP_RSP (message_p).bearer_s1u_enb_fteid.teid =
    htonl (*((uint32_t *) eRABSetupItemCtxtSURes_p->value.choice.E_RABSetupItemCtxtSURes.gTP_TEID.buf));
  memcpy (&MME_APP_INITIAL_CONTEXT_SETUP_RSP (message_p).bearer_s1u_enb_fteid.ipv4_address,
    eRABSetupItemCtxtSURes_p->value.choice.E_RABSetupItemCtxtSURes.transportLayerAddress.buf, 4);
  MSC_LOG_TX_MESSAGE (MSC_S1AP_MME,
                      MSC_MMEAPP_MME,
                      NULL, 0,
...
//------------------------------------------------------------------------------
int
s1ap_mme_handle_error_ind_message (const sctp_assoc_id_t assoc_id, const sctp_stream_id_t stream, S1AP_S1AP_PDU_t *pdu)
{
  OAILOG_FUNC_IN (LOG_S1AP);
